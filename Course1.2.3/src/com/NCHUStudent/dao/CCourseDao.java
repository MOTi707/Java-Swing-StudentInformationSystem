...
public class CCourseDao {
		...
		if(course_counts_index==-1){
			...
			case 1:
				...
				sql = "select temp.stu_id,course_id from ((select stu_id,count(*) as counts  from c_ccourse group by stu_id having counts=1) as temp),c_ccourse where c_ccourse.stu_id = temp.stu_id";
					...
					try {
						ResultSet rs1 = JDBCUtil.query(sql);
						while(rs1.next()){
							CCourseModel ccm = new CCourseModel();
			...
			case 2:
				sql = "select temp.stu_id,course_id from ((select stu_id,count(*) as counts  from c_ccourse group by stu_id having counts>=2) as temp),c_ccourse where c_ccourse.stu_id = temp.stu_id";
					...
					try {
						ResultSet rs2 = JDBCUtil.query(sql);
						while(rs2.next()){
							CCourseModel ccm = new CCourseModel();
	...
	public static List<Integer> getCCourseMarkByCourseId(int course_id){
		List<Integer> a = new ArrayList<>();
		String sql = "select ccourse_mark from c_ccourse where course_id="+course_id;
		ResultSet rs = JDBCUtil.query(sql);
		try{
			while(rs.next()){
				int ccourseMark = rs.getInt("ccourse_mark");
				a.add(ccourseMark);
			}
		}
		catch(SQLException e){
...
/**
 * @author Feng Xiong
 * @since 2024-7-19 16:00:02
 */
	public List<StuCourseMarkModel> getCCourseMarkCourseNameByStuId(int stu_id){
		List<StuCourseMarkModel> list = new ArrayList<>();
		String sql = "select * from c_ccourse where stu_id="+stu_id;
		ResultSet rs = JDBCUtil.query(sql);
		try{
			while (rs.next()){
				StuCourseMarkModel scm = new StuCourseMarkModel();
				scm.setStu_id(rs.getInt("stu_id"));
				scm.setCourse_id(rs.getInt("course_id"));
				scm.setCourseMark(rs.getInt("ccourse_mark"));
				list.add(scm);
			}
		} catch (SQLException e) {
			e.printStackTrace();
		...
		for(StuCourseMarkModel scm1:list){
			sql = "select course_name from c_course where course_id="+scm1.getCourse_id();
			try{
				rs = JDBCUtil.query(sql);
				if(rs.next()){
					scm1.setCourseName(rs.getString("course_name"));
				}
			} catch (SQLException e) {
				e.printStackTrace();
			}
